...
select t2.manageruesrname,t2.password 
from bondfund..userlogininfo t2 where t2.manageruesrname='{0}' and t2.password='{1}'
", this.userName, this.userPassword);
                 int rows = sqh.ExecuteDataSetText(sql_str, null).Tables[0].Rows.Count;
                 if (rows != 0 )
                 {
                     userID = 100;
                     //int id = userInfo.ID;
